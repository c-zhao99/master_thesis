...
CREATE TABLE Interract (
    
    Interract_ProductManager_EmployeeID {ADD_TYPE} UNIQUE NOT NULL REFERENCES Employee(EmployeeID),
    Interract_Customer_CustomerID {ADD_TYPE} UNIQUE NOT NULL REFERENCES Customer(CustomerID),
    PRIMARY KEY (Interract_ProductManager_EmployeeID,Interract_Customer_CustomerID)
);
...
CREATE TRIGGER InterractProductManager
BEFORE INSERT ON Interract
REFERENCING NEW AS N
FOR EACH ROW
WHEN (
    NOT EXISTS(
        SELECT * FROM Employee
        WHERE Employee.EmployeeID = N.Interract_ProductManager_EmployeeID
