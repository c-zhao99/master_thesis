CREATE TABLE Employee (
    ...
    CONSTRAINT Manager CHECK ((TYPE_Manager = 1 AND TeamName IS NOT NULL) OR (TYPE_Manager = 0 AND TeamName IS NULL AND PlanWith_Manager_EmployeeID IS NULL AND PlanWith_Manager_Year IS NULL AND Promote_Engineer_EmployeeID IS NULL AND Manage_Employee_EmployeeID IS NULL)),
    CONSTRAINT Engineer CHECK ((TYPE_Engineer = 1 AND Level IS NOT NULL) OR (TYPE_Engineer = 0 AND Level IS NULL)),
    CONSTRAINT ProductManager CHECK ((TYPE_ProductManager = 1 AND Product IS NOT NULL) OR (TYPE_ProductManager = 0 AND Product IS NULL AND Interract_Customer_EmployeeID IS NULL))
);
