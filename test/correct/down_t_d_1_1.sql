...
CREATE TABLE GovOfficial (
    ...
    Interract_Manager_Year {ADD_TYPE} NOT NULL,
    Interract_Engineer_Year {ADD_TYPE} NOT NULL,
    Interract_Manager_EmployeeID {ADD_TYPE} UNIQUE NOT NULL REFERENCES Manager(EmployeeID),
    Interract_Engineer_EmployeeID {ADD_TYPE} UNIQUE NOT NULL REFERENCES Engineer(EmployeeID),
    PRIMARY KEY (GovID),
    CONSTRAINT Interract CHECK ((Interract_Manager_EmployeeID IS NOT NULL AND Interract_Manager_Year IS NOT NULL AND Interract_Engineer_EmployeeID IS NULL AND Interract_Engineer_Year IS NULL) OR (Interract_Engineer_EmployeeID IS NOT NULL AND Interract_Engineer_Year IS NOT NULL AND Interract_Manager_EmployeeID IS NULL AND Interract_Manager_Year IS NULL))
);
