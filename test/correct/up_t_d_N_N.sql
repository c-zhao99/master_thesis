CREATE TABLE Employee (
    ...
    CONSTRAINT ManagerEngineerProductManager CHECK ((TYPE_ManagerEngineerProductManager = Manager AND TeamName IS NOT NULL AND Promote_Engineer_EmployeeID IS NOT NULL AND Level IS NULL AND Product IS NULL AND Manage_Employee_EmployeeID IS NOT NULL) OR (TYPE_ManagerEngineerProductManager = Engineer AND Level IS NOT NULL AND Promote_Engineer_EmployeeID IS NULL AND TeamName IS NULL AND Product IS NULL AND Manage_Employee_EmployeeID IS NULL) OR (TYPE_ManagerEngineerProductManager = ProductManager AND Product IS NOT NULL AND Promote_Engineer_EmployeeID IS NULL AND TeamName IS NULL AND Level IS NULL AND Manage_Employee_EmployeeID IS NULL))
);
